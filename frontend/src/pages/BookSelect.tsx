...
export default function BookSelect() {
  ...
  const autoAnnounceMessage = books.length > 0
    ? `교재 선택. ${books.length}개의 교재가 있습니다. 1부터 ${Math.min(books.length, 9)}까지 번호를 눌러 선택하세요.`
    : '교재 선택. 등록된 교재가 없습니다. 관리자에게 문의하세요.';
  ...
  return (
    <AppShellMobile title="교재 선택" className="relative h-screen flex flex-col">
      <div className="flex-1 flex flex-col px-6 py-8 overflow-y-auto">
        ...
        {/* 문학 · 수학 · 영어 */}
        <div className="mb-6">
          <h2 className="text-lg font-semibold mb-3">과목</h2>
          <div className="grid grid-cols-3 gap-3">
            <button
              onClick={() => navigate('/literature/lectures')}
              className="p-4 rounded-xl border-2 border-red-200 bg-gradient-to-br from-red-50 to-red-100 hover:border-red-300 hover:shadow-md transition-all text-left"
              aria-label="문학 강의"
            >
              <span className="text-2xl block mb-1">📚</span>
              <span className="font-semibold text-fg">문학</span>
            </button>
            <button
              onClick={() => navigate('/math1/lectures')}
              className="p-4 rounded-xl border-2 border-green-200 bg-gradient-to-br from-green-50 to-green-100 hover:border-green-300 hover:shadow-md transition-all text-left"
              aria-label="수학1 강의"
            >
              <span className="text-2xl block mb-1">📐</span>
              <span className="font-semibold text-fg">수학</span>
            </button>
            <button
              onClick={() => navigate('/english/lectures')}
              className="p-4 rounded-xl border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100 hover:border-blue-300 hover:shadow-md transition-all text-left"
              aria-label="영어 강의"
            >
              <span className="text-2xl block mb-1">🌐</span>
              <span className="font-semibold text-fg">영어</span>
            </button>
          </div>
        </div>
        ...
        {books.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center">
            <p className="text-muted mb-2">등록된 교재가 없습니다.</p>
            <p className="text-sm text-muted-foreground">교재 추가는 관리자 페이지에서 가능합니다.</p>
          </div>
        ) : (
        ...
        <div className="mt-4 text-xs text-muted-foreground text-center">
          <p>
            키보드로 조작: 1-{Math.min(books.length, 9)} (선택), R (재파싱),
            <br />
            ↑↓ (이동), Enter (선택), B (뒤로)
            <br />
            <span className="text-primary">💡 파싱 완료된 교재는 "JSON 동기화" 버튼을 클릭하세요</span>
            <br />
            <span className="text-muted-foreground">💡 교재 추가는 관리자 페이지에서 가능합니다</span>
          </p>
        </div>
      </div>
