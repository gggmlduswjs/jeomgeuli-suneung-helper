...
import { math1API, type Math1LectureSummary } from '../services/math1';
import { createModuleLogger } from '../utils/logger';

const logger = createModuleLogger('Math1Lectures');

export default function Math1Lectures() {
  const navigate = useNavigate();
  const [lectures, setLectures] = useState<Math1LectureSummary[]>([]);
  ...
  const loadLectures = async () => {
    ...
    setError(null);
    try {
      const data = await math1API.getLectures();
      setLectures(data);
      logger.log(`수학1 강의 ${data.length}개 로드`);
      showToastMessage(`수학1 강의 ${data.length}개를 불러왔습니다.`);
      speak(`수학1 강의 ${data.length}개를 불러왔습니다.`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : '강의 목록을 불러오지 못했습니다.';
      setError(msg);
      logger.error('수학1 강의 로드 실패:', err);
  ...
  const handleLectureSelect = (lecture: Math1LectureSummary) => {
    showToastMessage(`${lecture.title}을(를) 선택했습니다.`);
    speak(`${lecture.title}을(를) 선택했습니다.`);
    stopTTS();
    stopSTT();
    navigate(`/math1/lectures/${lecture.lecture_id}`);
  };

  const handleLectureRead = (lecture: Math1LectureSummary) => {
    speak(`${lecture.lecture_id}강. ${lecture.title}`);
  };

  return (
    <AppShellMobile title="수학1 강의 목록" className="relative h-screen flex flex-col">
      <div className="mb-2">
        <SpeechBar isListening={isListening} transcript={transcript} />
      </div>
      <div className="px-2 py-1 space-y-2 flex-1 overflow-y-auto min-h-0">
        <div className="bg-card border border-border rounded-lg p-4">
          <h2 className="text-xl font-bold mb-2">수학1 강의</h2>
        ...
        {!loading && !error && lectures.length > 0 && (
          <div className="space-y-2" role="list" aria-label="수학1 강의 목록">
            {lectures.map((l) => (
              <div
                key={l.lecture_id}
