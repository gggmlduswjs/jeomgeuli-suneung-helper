...
export function useArduinoButtons(): UseArduinoButtonsReturn {
  ...
  const portRef = useRef<any | null>(null); // SerialPort from Web Serial API
  ...
  const connect = useCallback(async () => {
    try {
      ...
      if (portRef.current && portRef.current.readable) {
        if (import.meta.env.DEV) console.log('[Arduino] 이미 연결된 포트가 있습니다.');
        return;
      }
      ...
      (async () => {
        try {
          ...
          if (error.name !== 'AbortError') {
            ...
            setError(`읽기 오류: ${error.message || '알 수 없는 오류'}`);
            disconnect();
          }
        }
      })();

      if (import.meta.env.DEV) console.log('[Arduino] 연결 성공');
    } catch (err: unknown) {
