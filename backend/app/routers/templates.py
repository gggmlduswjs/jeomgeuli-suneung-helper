...
from app.services.template_service import (
    ...
    _normalize_bbox_to_ratios,
    _extract_images_from_bbox_regions,
    _extract_text_from_bbox_regions,
    _compute_region_hints,
    _build_toc_prompt,
    _generate_template_from_toc_via_openai,
    _extract_lecture_lines_from_toc_directly,
    _validate_generated_toc_patterns,
    _extract_text_by_bbox,
    _extract_text_by_region_hints,
)
...
# Pydantic 모델들은 template_service에서 import (하위 호환성을 위해 라우터에도 남겨둠)
# 하지만 서비스에서 import한 것을 우선 사용
class ParsingGuideRegion(BaseModel):
    """파싱 가이드 영역 (YOLO-style bbox) - template_service에서 import"""
...
class CurriculumStructureSurvey(BaseModel):
    """커리큘럼 구조 설문 - template_service에서 import"""
