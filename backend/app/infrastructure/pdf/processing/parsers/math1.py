"""
수학 파서 (래핑 버전)
기존 Math1ParsingStrategy를 래핑하여 사용
"""
import json
import logging
...
class Math1Parser(BaseParser):
    ...
    def parse(self, ocr_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """OCR 데이터 파싱"""
        from app.parsing.strategies.math1_strategy import Math1ParsingStrategy

        strategy = Math1ParsingStrategy()
        lectures = strategy.extract_lectures(ocr_data, self.config)
        problems = strategy.extract_problems(ocr_data, self.config)

        logger.info(f"수학 파싱 완료: {len(lectures)}개 강의, {len(problems)}개 문제")
