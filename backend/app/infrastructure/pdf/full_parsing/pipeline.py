    ...
    def process(self, pdf_path: Path) -> ParsingResult:
        ...
        try:
            ...
            if isinstance(self.extractor, OCRExtractor):
                ...
                try:
                    first_page = effective_start_page or 1
                    logger.info(f"   pdfplumber 모드: 추출 중 (페이지 {first_page}~{effective_end_page or '끝'})")
            ...
            # 템플릿 재로드는 템플릿 변경 시에만 수행 (성능 최적화)
            # 템플릿이 이미 로드되어 있으면 재로드 생략
            if not self.hybrid_router.template_manager.templates:
                logger.debug("   템플릿이 없어 재로드 수행")
                self.hybrid_router.template_manager.reload_templates()
            else:
                logger.debug(f"   템플릿 {len(self.hybrid_router.template_manager.templates)}개 이미 로드됨 (재로드 생략)")
            try:
