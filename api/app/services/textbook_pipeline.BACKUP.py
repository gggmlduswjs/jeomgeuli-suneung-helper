...
from app.extraction.extractors import PdfplumberExtractor, OCRExtractor, TextExtractor
from app.extraction.image_processor import ImageProcessor
from app.parsing.utils import group_texts_by_line, matches_patterns
from app.parsing.strategies import LiteratureParsingStrategy, Math1ParsingStrategy, EnglishParsingStrategy
...
class TextbookPipeline:
    ...
    def __init__(
        ...
        if subject == 'literature':
            self.parsing_strategy = LiteratureParsingStrategy()
        elif subject == 'math1':
            self.parsing_strategy = Math1ParsingStrategy()
        elif subject == 'english':
            self.parsing_strategy = EnglishParsingStrategy()
        else:
            # 기본값: 문학
            self.parsing_strategy = LiteratureParsingStrategy()
    ...
    def _preprocess_image(self, image: Image.Image) -> Image.Image:
        """이미지 전처리 (OCR 정확도 향상) - ImageProcessor 사용"""
    ...
    def _process_and_save_image(self, image: Image.Image, page_num: int) -> Image.Image:
        """이미지 전처리 및 저장 - ImageProcessor 사용"""
        return ImageProcessor.process_and_save_image(image, page_num, self.pages_dir)
    ...
    def _extract_lectures(self, all_ocr_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        강의/단원 목록 자동 생성 (전략 패턴 사용)
        
        전략 패턴이 구현되지 않은 경우 기존 메서드 사용 (하위 호환성)
        """
        # 전략 패턴 사용 시도
        try:
            lectures = self.parsing_strategy.extract_lectures(all_ocr_data, self.config)
            if lectures:
                return lectures
        except Exception as e:
            logger.warning(f"전략 패턴 사용 실패, 기존 로직 사용: {e}")
    ...
    def _extract_problems(
        ...
        # 전략 패턴 사용 시도
        try:
            problems = self.parsing_strategy.extract_problems(all_ocr_data, self.config)
            if problems:
                # 증분 파싱: 기존 문제 제외
                new_problems = [p for p in problems if f"{p.get('page', 0):02d}_{p.get('problem_id', '')}" not in existing_problem_keys]
